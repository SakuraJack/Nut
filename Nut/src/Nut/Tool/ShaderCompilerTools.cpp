...
#include "Nut/Utils/StringUtils.h"
...
bool Nut::ShaderCompiler::Reload(bool forceCompile)
{
	...
	if (source.empty()) {
		NUT_CORE_ERROR_TAG("Shader", "加载着色器文件失败: {0}", m_ShaderSourcePath);
		return false;
	}

	NUT_CORE_TRACE_TAG("Shader", "加载着色器文件: {0}", m_ShaderSourcePath);
	...
	if (!compileSuccess) {
		NUT_CORE_ERROR_TAG("Shader", "编译着色器失败: {0}", m_ShaderSourcePath);
		return false;
	}
...
void Nut::ShaderCompiler::Reflect(std::vector<uint32_t>& data, std::string stage)
{
	...
	NUT_CORE_TRACE_TAG("Shader", "Shader Name - {0}, Shader Stage - {1}", m_ShaderSourcePath.filename(), stage);
	NUT_CORE_TRACE_TAG("Shader", "Uniform Buffers: {0}", resources.uniform_buffers.size());
	NUT_CORE_TRACE_TAG("Shader", "Storage Buffers: {0}", resources.storage_buffers.size());
	NUT_CORE_TRACE_TAG("Shader", "Sampled Images: {0}", resources.sampled_images.size());
	NUT_CORE_TRACE_TAG("Shader", "Plain Uniforms: {0}", resources.gl_plain_uniforms.size());
	...
	for (auto& [stage, source] : m_ShaderSource) {
		...
		if (shaderBinaries[stage].empty()) {
			...
			if (module.GetCompilationStatus() != shaderc_compilation_status_success) {
				NUT_CORE_ERROR_TAG("Shader", "着色器编译失败: {0}", module.GetErrorMessage());
				return false;
			}
