#include "ntpch.h"
#include "Texture.h"
#include "Nut/Asset/TextureImporter.h"
#include "Nut/Utils/ImageUtils.h"
#include "Nut/Utils/FileUtils.h"
#include "Renderer.h"
#include "glad/glad.h"
...
{
	if (imageData.size() != 6) {
		NUT_CORE_ERROR_TAG("Texture", "立方体纹理必须有六幅图像, 但只获得了 {0}", m_ImageDatas.size());
		return;
	}
	...
	: m_Specification(spec), m_FilePath(filepath)
{
	...
	if (files.size() != 6) {
		NUT_CORE_ERROR_TAG("Texture", "立方体纹理必须有六幅图像, 但只获得了 {0}", files.size());
		return;
	}
...
{
	if (faceindex > 6) {
		NUT_CORE_ERROR_TAG("Texture", "立方体纹理的面索引超出范围: {0}", faceindex);
		return;
	}
