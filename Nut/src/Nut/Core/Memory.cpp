...
namespace Nut {
	...
	static Nut::AllocationStats s_GlobalStats;
...
#if NT_ENABLE_PROFILING
		TracyAlloc(meomry, size);
#endif
	...
	void* Allocator::Allocate(size_t size, const char* file, const char* line)
	{
		if (!s_Data) Init();
...
#ifndef NT_DIST
			if (!found)
				NUT_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
#if NT_TRACK_MEMORY && NT_PLATFORM_WINDOWS
